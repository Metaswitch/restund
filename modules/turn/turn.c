...
static int module_init(void)
{
        ...
        /* turn_relay_snat_addr */
        if (!conf_get(restund_conf(), "turn_relay_snat_addr", &opt)) {
                turnd.snat = true;
                err = sa_set(&turnd.snat_rel_addr, &opt, 0);
                restund_info("turn: server deployed behind static NAT addr=%J\n", &turnd.snat_rel_addr);
        } else {
                turnd.snat = false;
        }

        if (err) {
                restund_error("turn: bad turn_relay_snat_addr\n");
                goto out;
        }
        ...
        turnd.extended_channels = false;
        conf_get_bool(restund_conf(), "turn_extended_channels", &turnd.extended_channels);
        if (turnd.extended_channels)
        {
          restund_info("turn: extended channels enabled\n");
        }
