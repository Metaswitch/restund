                      ...
                      const struct stun_msg *msg)
{
        ...
        restund_info("turn: allocation request (src=%J, dst=%J)\n", src, dst);
        ...
        restund_info("turn: relay address %J\n", rel_addr);
        ...
        if (turnd->snat) {
                // Server is behind a static NAT, so advertise allocation using
                // NATed address
                restund_info("Advertising SNATed address for relay\n");
                al->advertised_rel_addr = turnd->snat_rel_addr;
                sa_set_port(&al->advertised_rel_addr, sa_port(&al->rel_addr));
        } else {
                al->advertised_rel_addr = al->rel_addr;
        }

        restund_debug("turn: allocation %p created %s/%J/%J - %J (%us)\n",
                      al, net_proto2name(al->proto), &al->cli_addr,
                      &al->srv_addr, &al->advertised_rel_addr, lifetime);
 ...
 reply:
        ...
        err = rerr = stun_reply(proto, sock, src, 0, msg,
                                ...
                                STUN_ATTR_XOR_RELAY_ADDR, &alx->advertised_rel_addr,
